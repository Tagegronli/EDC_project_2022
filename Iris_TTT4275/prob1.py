...
def prob1(train, test):
    ...
    # Simulating for different step sizes
    for step in steps:
    ...
    # Plotting MSE for training set
    plt.figure(1)
    plt.grid()
    plt.legend()
    plt.title('MSE for training set at iterations = ' + str(iterations) +
    ...
    ax.set_title('Confusion Matrix for training set @ step factor = ' + str(steps[1]) + ', iterations = '+ str(iterations) +
                 ',\n training samples = ' + str(train) + ', testing samples = ' + str(test) + '\n')
    ax.set_xlabel('\nClassified Category')
